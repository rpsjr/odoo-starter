...
RUN unzip -q odoo.zip && rm odoo.zip && mv odoo-13.0 /odoo/local-src/odoo && \
    ...
    unzip -q account-financial-reporting.zip && rm account-financial-reporting.zip && mv account-financial-reporting-13.0 /odoo/local-src/account-financial-reporting && \
    unzip -q mis-builder.zip && rm mis-builder.zip && mv mis-builder-13.0 /odoo/local-src/mis-builder && \
		unzip -q bank-payment.zip && rm bank-payment.zip && mv bank-payment-13.0 /odoo/local-src/bank-payment && \
		unzip -q account-payment.zip && rm account-payment.zip && mv account-payment-13.0 /odoo/local-src/account-payment && \
		unzip -q account-financial-tools.zip && rm account-financial-tools.zip && mv account-financial-tools-13.0 /odoo/local-src/account-financial-tools && \
		unzip -q server-tools.zip && rm server-tools.zip && mv server-tools-13.0 /odoo/local-src/server-tools && \
    unzip -q helpdesk.zip && rm helpdesk.zip && mv helpdesk-13.0 /odoo/local-src/helpdesk && \
...
RUN unzip -q website.zip && rm website.zip && mv website-13.0 /odoo/external-src/website && \
		unzip -q muk_base.zip && rm muk_base.zip && mv muk_base-13.0 /odoo/external-src/muk_base && \
		unzip -q muk_web.zip && rm muk_web.zip && mv muk_web-13.0 /odoo/external-src/muk_web && \
		unzip -q fleet_management.zip && rm fleet_management.zip && mv fleet_management-13.0 /odoo/external-src/fleet_management && \
